...
def main():
    ...
    argparser.add_argument(
        ...
        default=[2, 10],
        type=list,
        help='Next offset distance reset time')
    ...
    try:
        ...
        # Attach the collision sensor to the vehicle
        collision_bp = world.get_blueprint_library().find('sensor.other.collision')
        collision_sensor = world.spawn_actor(collision_bp, carla.Transform(), attach_to=hero_list[0])
        ...
        hero_offset = np.zeros_like(time_interval)
        ...
        normal_offset = np.zeros((len(normal_list), len(time_interval)))
        while True:
            if not args.asynch and synchronous_master:
                world.tick()

                if count == next_offset_time:
                    # offset = 4
                    ...
                    traffic_manager.vehicle_lane_offset(hero_list[0], offset)
                    # set the offset distance reset time for the next time
                    next_offset_time = count + int(random.uniform(args.t_next[0], args.t_next[1]) / settings.fixed_delta_seconds)
                ...
                if count >= experiment_time * 60 / settings.fixed_delta_seconds:
                    ...
                    ax5.plot(time_interval, hero_offset, label='ego_vehicle')
                    for i in range(len(normal_list)):
                        ax5.plot(time_interval, normal_offset[i], label='normal_' + str(i))
                    ax5.legend()
                    ax5.set_xlabel('Time (s)')
                    ax5.set_ylabel('Offset')
                    ax5.set_title('Offset-Time Graph')
                    plt.show()
                    break
